...
export default function DashboardPage() {
  ...
  const handleSaveTitle = async () => {
    ...
    if (originalQuery.title === newTitle) {
      setEditingTitle({ id: null, text: "" }); // No change, just close input
      return;
    }
    ...
    setEditingTitle({ id: null, text: "" }); // Close the input

    try {
      // Call the new API route to save the change
      ...
      if (!res.ok) {
        // If the save fails, revert the change in the UI
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          {/* --- MODIFIED EDITABLE TITLE SECTION --- */}
                          {editingTitle.id === query.id ? (
                            // Show input field when this item is being edited
                            <input
                              type="text"
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  e.preventDefault();
                                  handleSaveTitle(); // Save on Enter
                                }
                                if (e.key === "Escape")
                                  setEditingTitle({ id: null, text: "" }); // Cancel on Escape
                              }}
                              className="font-medium text-gray-800 p-1 border border-blue-500 rounded w-full"
                              autoFocus
                            />
                          ) : (
                            // Show normal title and edit button
                            <>
                              <p
